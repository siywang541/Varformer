...
from .var_sr_model import VarSRModel
...
class VarCodeFormerIdxModel(VarSRModel):
    ...
    def init_training_settings(self):
        ...
        # define losses
        self.cri_pix = train_opt.get('use_pixel_opt', True)
    ...
    def optimize_parameters(self, current_iter):
        ...
        if not self.cri_pix:
            ...
            output, logits, lq_feat = self.net_g(self.input,code_only=True, pixel_l = True)
    ...
    def test(self):
        with torch.no_grad():
            if hasattr(self, 'net_g_ema'):
                self.net_g_ema.eval()
                self.output, _, _ = self.net_g_ema(self.input)
            else:
                logger = get_root_logger()
                logger.warning('Do not have self.net_g_ema, use self.net_g.')
                self.net_g.eval()
                self.output, _, _ = self.net_g(self.input)
                self.net_g.train()
    ...
    def nondist_validation(self, dataloader, current_iter, tb_logger, save_img):
        ...
        for idx, val_data in enumerate(dataloader):
            ...
            if save_img:
                ...
                imwrite(gt_img, save_img_path.replace('.png','__.png'))
...
class VarCodeFormerIdxModel3(VarSRModel):
    ...
    def init_training_settings(self):
        ...
        self.hq_feat_loss = train_opt.get('use_hq_feat_loss', True)
        self.feat_loss_weight = train_opt['feat_loss_weight']#.get('feat_loss_weight', 1.0)
        self.cross_entropy_loss = train_opt.get('cross_entropy_loss', True)
        self.entropy_loss_weight = train_opt['entropy_loss_weight']#.get('entropy_loss_weight', 0.5)
        self.perceptual_loss_weight = train_opt['perceptual_loss_weight']
        # define losses
        self.cri_pix = train_opt.get('use_pixel_opt', True)
    ...
    def optimize_parameters(self, current_iter):
        ...
        if not self.cri_pix:
            ...
            output, lq_feat = self.net_g(self.input,self.gt2,code_only=True, pixel_l = True)
        ...
        if self.cri_perceptual:
            l_g_percep = self.cri_perceptual(output, self.gt)[0]* self.perceptual_loss_weight
            l_g_total += l_g_percep
            loss_dict['l_g_percep'] = l_g_percep

        # perceptual loss
        if self.cri_perceptual:
            l_g_psnr = self.cri_psnr(output, self.gt) #* self.perceptual_loss_weight
            l_g_total += l_g_psnr
            loss_dict['l_g_psnr'] = l_g_psnr
    ...
    def test(self):
        with torch.no_grad():
            if hasattr(self, 'net_g_ema'):
                self.net_g_ema.eval()
                self.output, _ = self.net_g_ema(self.input, self.gt2)
            else:
                logger = get_root_logger()
                logger.warning('Do not have self.net_g_ema, use self.net_g.')
                self.net_g.eval()
                self.output, _ = self.net_g(self.input, self.gt2)
                self.net_g.train()
...
class VarCodeFormerIdxModel4_2(VarSRModel):
    ...
    def optimize_parameters(self, current_iter):
        ...
        # SSIM loss
        if self.cri_ssim:
            l_g_ssim = self.cri_ssim(output, self.gt) #* self.perceptual_loss_weight
            l_g_total += l_g_ssim
    ...
    def test(self):
        with torch.no_grad():
            if hasattr(self, 'net_g_ema'):
                self.net_g_ema.eval()
                self.output, _ = self.net_g_ema(self.input)
            else:
                logger = get_root_logger()
                logger.warning('Do not have self.net_g_ema, use self.net_g.')
                self.net_g.eval()
                self.output, _ = self.net_g(self.input)
                self.net_g.train()
...
class VarCodeFormerIdxModel4_2_del(VarSRModel2): 
    ...
    def init_training_settings(self):
        ...
        if self.ema_decay > 0:
            ...
            load_path = self.opt['path'].get('pretrain_network_g', None)
            if load_path is not None:
                key = self.load_network(self.net_g_ema, load_path, self.opt['path'].get('strict_load_g', True), 'params_ema', del_module=True,del_m_name='AFT')
            else:
                self.model_ema(0)  # copy net_g weight
            self.net_g_ema.eval()
...
class VarCodeFormerIdxModel4_2_losszd(VarSRModel): 
    ...
    def init_training_settings(self):
        ...
        self.hq_feat_loss = train_opt.get('use_hq_feat_loss', True)
        f_l_w = train_opt['feat_loss_weight']#.get('feat_loss_weight', 1.0)
        self.feat_loss_weight = nn.Parameter(torch.tensor(f_l_w))

        self.cross_entropy_loss = train_opt.get('cross_entropy_loss', True)
        e_l_w = train_opt['entropy_loss_weight']#.get('entropy_loss_weight', 0.5)
        self.entropy_loss_weight = nn.Parameter(torch.tensor(e_l_w))
        
        p_l_w = train_opt['perceptual_loss_weight']
        self.perceptual_loss_weight = nn.Parameter(torch.tensor(p_l_w))
        # define losses
        self.cri_pix = train_opt.get('use_pixel_opt', True)
        ...
        if train_opt.get('psnr_opt'):
            self.cri_psnr = build_loss(train_opt['psnr_opt']).to(self.device)
            self.psnr_l_w = nn.Parameter(torch.tensor(0.5))
        else:
            self.cri_psnr = None
        ...
        if train_opt.get('ssim_opt'):
            self.cri_ssim = build_loss(train_opt['ssim_opt']).to(self.device)
            self.s_l_w = nn.Parameter(torch.tensor(0.5))
        else:
            self.cri_ssim = None
    ...
    def optimize_parameters(self, current_iter):
        ...
        # PSNR loss
        if self.cri_psnr:
            l_g_psnr = self.cri_psnr(output, self.gt)*self.psnr_l_w #* self.perceptual_loss_weight
            l_g_total += l_g_psnr
            loss_dict['l_g_psnr'] = l_g_psnr

        # SSIM loss
        if self.cri_ssim:
            l_g_ssim = self.cri_ssim(output, self.gt)*self.s_l_w #* self.perceptual_loss_weight
            l_g_total += l_g_ssim
            loss_dict['l_g_ssim'] = l_g_ssim
