...
class VectorQuantizer2(nn.Module):
    ...
    def __init__(
        self, vocab_size, Cvae, using_znorm, beta: float = 0.25, default_qresi_counts=0, v_patch_nums=None, quant_resi=0.5, share_quant_resi=4):
        super().__init__()
    ...
    def embed_to_fhat2(self, ms_h_BChw: List[torch.Tensor], all_to_max_scale=True, last_one=False,layer_ls=[]) -> Union[List[torch.Tensor], torch.Tensor]:
        ls_f_hat_BChw = []
        B = ms_h_BChw[0].shape[0]
        H = W = self.v_patch_nums[-1]
        SN = len(self.v_patch_nums)
    ...
    def idxBl_to_var_input(self, gt_ms_idx_Bl: List[torch.Tensor]) -> torch.Tensor:
        ...
        return first_h_BChw, f_hat, torch.cat(next_scales, dim=1) if len(next_scales) else None    # cat BlCs to BLC, this should be float32
    ...
    def get_next_autoregressive_input_h(self, si: int, SN: int, f_hat: torch.Tensor, h_BChw: torch.Tensor) -> Tuple[Optional[torch.Tensor], torch.Tensor]: # only used in VAR inference
        HW = self.v_patch_nums[-1]
    ...
    def get_next_autoregressive_input2(self, si: int, SN: int, f_hat: torch.Tensor, h_BChw: torch.Tensor, first_h_BChw =None) -> Tuple[Optional[torch.Tensor], torch.Tensor]: # only used in VAR inference
        HW = self.v_patch_nums[-1]
        if si == 0:
            f_hat.add_(self.quant_resi[si/(SN-1)](first_h_BChw))     
            return f_hat, F.interpolate(f_hat, size=(self.v_patch_nums[si+1], self.v_patch_nums[si+1]), mode='area')
