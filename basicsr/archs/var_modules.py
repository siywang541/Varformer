...
class SelfAttention(nn.Module):
    ...
    def forward(self, x, attn_bias, if_mix=False):
        B, L, C = x.shape
        ...
        if self.caching:
            if self.cached_k is None: self.cached_k = k; self.cached_v = v #; print('cach is None !!!!!!!!!!!!!!!!!!!!!!!!!')
...
class AdaLNSelfAttn(nn.Module):
    ...
    # NOTE: attn_bias is None during inference because kv cache is enabled
    def forward(self, x, cond_BD, attn_bias, if_mix=False):   # C: embed_dim, D: cond_dim
        if self.shared_aln:
...
class CrossAttention1(nn.Module):
    ...
    def forward(self, f_d, f_var):
        B, N, C = f_d.shape

        f_q = self.norm_q(f_d)
        f_q = self.linear_q(f_q)

        f_kv = self.linear_kv(f_var)

        f_k = self.norm_k(f_kv)
        f_k = self.linear_k(f_k)

        f_v = self.norm_k(f_kv)
        f_v = self.linear_k(f_v)


        attn_weights = torch.bmm(f_q, f_k.transpose(1, 2)) / (self.dim ** 0.5)

        attn_weights = F.softmax(attn_weights, dim=-1)

        attn_output = torch.bmm(attn_weights, f_v)

        output = f_d + self.linear_c(attn_output)
...
class CrossAttentionAR(nn.Module):
    def __init__(
        self, block_idx, dim=1024, num_heads=16,
        attn_drop=0., proj_drop=0., attn_l2_norm=False, flash_if_available=True,
    ):
