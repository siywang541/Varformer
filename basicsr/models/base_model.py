...
class BaseModel():
    ...
    def load_network(self, net, load_path, strict=True, param_key='params_ema', del_module=False,del_m_name=None):
        """Load network.
        ...
        for k, v in deepcopy(load_net).items():
            if k.startswith('module.'):
                ...
                load_net.pop(k)
            if del_module == True:
                if k.startswith(del_m_name):
                    load_net.pop(k)
                    
        self._print_different_keys_loading(net, load_net, strict)
        key = net.load_state_dict(load_net, strict=strict)
        return key
